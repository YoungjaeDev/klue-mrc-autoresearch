...
- 루프 전체에는 시간 상한이 없다. `/goal clear`까지 돈다. 40분 제한은 학습이나 채점 실행 1회에만 걸리는 멈춤 방지 규칙이다.
- 후보 1개 = 학습(128 step, 중간에 32 step마다 고정 128행 eval/loss) + 새 프로세스에서 search 1,172문항 생성·채점. 대부분의 시간은 채점이 차지한다.
- 정해진 시간만 돌리려면 /goal 제약에 "시작 후 N시간이 지나면 진행 중인 후보를 마치고 멈춘다"를 덧붙인다.
