...
```powershell
uv run python -m autoresearch_lab.run --state-dir outputs/state --run-dir outputs/base-search/process --timeout-seconds 3600 --gpu --frozen-manifest $manifest -- $studioPython -B -m autoresearch_lab.bootstrap --overlay $overlay --cache-dir outputs/base-search/cache --module rehearsal.official_mrc_runner -- run --model $model --revision $revision --tokenizer $model --tokenizer-revision $revision --device cuda --dtype bfloat16 --load-in-4bit --max-length 4096 --max-new-tokens 128 --split-manifest $splitPath --split-manifest-sha256 $splitHash --split search --output outputs/base-search/eval
```
