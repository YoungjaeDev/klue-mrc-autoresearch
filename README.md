...
| 파일 | 하는 일 | 연구 중 변경 |
| --- | --- | --- |
| `README.md` | 사람이 설치·준비·실행 순서를 읽습니다. | 사용 안내 |
...
네 파일은 처음 읽을 입구입니다. 실제 실행에는 `autoresearch_lab/`, 공식 평가 코드, `pyproject.toml`·`uv.lock` 등의 보조 파일도 필요하므로 전체 저장소를 clone하세요. 이 저장소는 원본의 GitHub fork가 아니라 원본 연구 흐름을 참고한 독립 Unsloth 실습 구현입니다.
...
강의에서는 Claude Code로 연구를 맡기는 흐름을 설명합니다. 설치·로그인과 위 환경 준비를 마친 뒤 저장소 폴더에서 `claude --permission-mode default`를 실행하세요. [Claude Code 시작 안내](docs/claude-code.md)에 지침 로드 확인, 첫 요청과 재개 방법을 정리했습니다. Codex 등 다른 코딩 에이전트도 같은 `program.md`를 사용할 수 있습니다.

현재 10후보의 비공개 연구 캠페인은 Codex가 지휘했습니다. Claude Code로 같은 연구를 수행한 리허설은 아직 하지 않았습니다. 아래는 새 실습을 시작할 때 전달할 요청입니다.
